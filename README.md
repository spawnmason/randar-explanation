...
"Randar" is an exploit for Minecraft which uses [LLL lattice reduction](https://en.wikipedia.org/wiki/Lenstra%E2%80%93Lenstra%E2%80%93Lov%C3%A1sz_lattice_basis_reduction_algorithm) to crack the internal state of an incorrectly reused `java.util.Random` in the Minecraft server, then works backwards from that, due to a flaw in how the RNG is seeded, to locate other players currently loaded into the world.
