...
The lattice pattern in 2d as lines is even more apparent in 3d as planes. (aside: this is actually [a test for PRNGs](https://en.wikipedia.org/wiki/Spectral_test), whether or not it makes planes like this, and if so, how far apart they are). And still keep in mind that this GIF is NOT the real 3d plot, it's just a simplification, the real 3d plot has nearly 300 trillion points. 
...
Here's a visualization of the problem: we have our lattice, and our measurements tell us the coordinates of the cube we need to search in (shown in red), now the problem is efficiently identifying a lattice point within that cube.
